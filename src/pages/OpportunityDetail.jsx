...
import { Eye, Calendar, MapPin, Clock, Settings2, Send, Rocket, Pause, Play, Check } from 'lucide-react';
...
export default function OpportunityDetail() {
  ...
  const { getOpportunity, getCandidates, setOppStatus } = useApp();
    ...
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 22 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          {opp.status === 'DRAFT' && <button className="btn-success" onClick={() => { setOppStatus(id, 'OPEN'); showToast('Opportunity published — now live'); }}><Rocket size={15} /> Publish</button>}
          {opp.status === 'OPEN' && <button className="btn-ghost" onClick={() => { setOppStatus(id, 'PAUSED'); showToast('Opportunity paused'); }}><Pause size={15} /> Pause</button>}
          {(opp.status === 'PAUSED' || opp.status === 'CLOSED') && <button className="btn-success" onClick={() => { setOppStatus(id, 'OPEN'); showToast('Opportunity reopened'); }}><Play size={15} /> {opp.status === 'CLOSED' ? 'Reopen' : 'Resume'}</button>}
      ...
      {opp.assessment && (
        <div className="card" style={{ padding: '22px 24px', marginBottom: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <h2 style={{ fontSize: 17, fontWeight: 700, margin: 0 }}>Assessment <span style={{ color: '#9CA3AF', fontWeight: 600, fontSize: 13 }}>— what candidates run</span></h2>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
            {opp.assessment.modules.map((m, i) => (
              <span key={i} className="chip" style={{ background: '#F8FAFF', color: '#056FD4', border: '1px solid #E0EDFF' }}>{i + 1}. {MODNAME[m.key] || m.key}{m.weight ? ` · ${m.weight}%` : ''}</span>
            ))}
          </div>
