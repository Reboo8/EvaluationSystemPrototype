    ...
    <>
      ...
      <div className="card" style={{ overflow: 'auto' }}>
        {agents.length === 0 ? (
          ...
          <table>
            <thead><tr><th style={{ width: 36 }}></th><th>Rank</th><th>Candidate</th><th>Weighted</th>{weights.map((w) => <th key={w.label}>{w.label}</th>)}<th>CEFR</th></tr></thead>
            <tbody>
              {agents.map((a, i) => {
                ...
                return (
                  <tr className="row" key={a.id}>
                    ...
                    <td><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><div className="avatar" style={{ width: 32, height: 32, background: bg, color: fg, fontSize: 11 }}>{ini(a.name)}</div><span style={{ fontWeight: 600, color: '#056FD4', cursor: 'pointer' }} onClick={() => nav(`/opportunities/${id}/candidate/${a.id}`)}>{a.name}</span></div></td>
                    <td style={{ fontWeight: 700, color: '#059669' }}>{weightedScore(a, weights)}</td>
                    {weights.map((w) => <td key={w.label}>{a.scores?.[w.label] ?? '—'}</td>)}
      ...
      {agents.length > 0 && <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 10 }}>Tick 2+ candidates, then “Compare” for a side-by-side.</div>}
    </>
  );
}
